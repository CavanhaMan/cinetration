...
/*LISTAR NOME E GENERO DE TODOS OS FILMES, POR ORDEM DE GENERO*/
...
/*LISTAR NOME E GENERO DE TODOS OS FILMES DE AÇÃO POR ORDEM DE FILME*/
select distinct f.nome_film FILME, g.nome_gen GENERO
from fgenero fg
inner join filme f on f.cod_film = fg.cod_film
inner join genero g on g.cod_gen = fg.cod_gen
...
*/
/*QUANTOS FILMES DE DRAMA A LISTA POSSUI?*/
...
/*QUANTOS DIRETORES CADASTRADOS?*/
select nome_dir from diretor;
select count(*) from diretor;

/*QUAIS DIRETORES ESTÃO EM MAIS DE 1 FILME?*/
...
/*QUAL O DIRETOR QUE TEM MAIS FILME?*/
...
/*QUANTOS IDIOMAS TEM OS FILMES DESSA LISTA?*/
SELECT count(*) from lingua;

#SELECT version();

/*QUAIS FILMES TEM DATA DE LANCAMENTO ZERADA?*/
select nome_film FILME, release_date DATA_LANCAMENTO
from filme
having release_date = (select MIN(release_date) from filme);

/*QUAIS FILMES FALAM Mandarin?*/
...
/*MOSTRE NOME E LINGUAGEM DE TODOS OS FILMES, ORGANIZADO POR LINGUA*/
...
/*QUAL O FILME QUE TEM MAIS IDIOMAS FALADOS?*/
...
/* 18 QUAIS FILMES FORAM LANCADOS EM 2020 NA AMERICA? */
...
/* 36 QUAL O FILME QUE TEM MAIOR CLASSIFICACAO PELO IMDB E QUAL O LINK DELE NO IMDB? */
select max(vote_avg)
from filme;
